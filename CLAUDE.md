...
**Current version:** `2.40.12` (see `.claude-plugin/plugin.json` `version` field — canonical source of truth for the plugin marketplace. `CHANGELOG.md` tracks the release history).
...
- **Plan-review panel centralization** — `plan-review` runs up to 3 reviewers (qa ratify-plan → security design-review conditional → plan-reviewer last) into ONE `01-plan.md`; zero side-files; one `## Plan Review` + deterministic worst-of combined verdict (`fail > concerns > pass`); preserve-in-place of upstream sub-verdicts; vacuous-success guard (missing-but-expected label → panel incomplete, not auto-pass). See `agents/ref-direct-modes.md`.
